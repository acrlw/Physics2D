...
namespace Physics2D
{
	...
	std::vector<Body*> DBVT::raycast(const Vector2& point, const Vector2& direction)
	{
	...
	std::vector<std::pair<Body*, Body*>> DBVT::generate()
	{
	...
	void DBVT::insert(Body* body)
	{
		...
		upgrade(boxIndex);
		balance(m_rootIndex);
		for(auto& [body, index]: m_bodyTable)
		{
			upgrade(index);
		}
	...
	void DBVT::update(Body* body)
	{
		...
		thin.expand(0.1);
		if (!thin.isSubset(iter->first->aabb()))
		{
			extract(iter->second);
			insert(body);
		}
	...
	void DBVT::raycast(std::vector<Body*>& result, int nodeIndex, const Vector2& p, const Vector2& d)
	{
		if (nodeIndex < 0)
			return;
	...
	void DBVT::generate(int nodeIndex, std::vector<std::pair<Body*, Body*>>& pairs)
	{
	...
	void DBVT::generate(int leftIndex, int rightIndex, std::vector<std::pair<Body*, Body*>>& pairs)
	{
	...
	int DBVT::merge(int nodeIndex, int leafIndex)
	{
	...
	int DBVT::calculateLowestCostNode(int nodeIndex)
	{
	...
	void DBVT::upgrade(int nodeIndex)
	{
		if (nodeIndex == -1 || m_tree[nodeIndex].isLeaf())
			return;
