...
namespace Physics2D
{
	...
	public:
		...
		static std::tuple<CCD::BroadphaseTrajectory, AABB> buildTrajectoryAABB(Body* body, const real& dt);
		...
		static std::optional<std::tuple<real, Body*>> query(DBVH::Node* root, Body* body, const real& dt);

	private:
		static void queryNodes(DBVH::Node* node, const AABB& aabb, std::vector<DBVH::Node*>& nodes, Body* body = nullptr);
